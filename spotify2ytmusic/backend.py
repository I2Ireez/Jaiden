...
def iter_spotify_followed_artists(
    spotify_playlist_file: str = "playlists.json",
    spotify_encoding: str = "utf-8",
) -> Iterator[str]:
    """Get the list of followed artists from Spotify.

    This function reads the followed artists information from the playlists JSON file
    that was generated by spotify_backup.py.
    ...
    Yields:
        Iterator[str]: Name of each followed artist.
    """
    spotify_pls = load_playlists_json(spotify_playlist_file, spotify_encoding)

    if "followed_artists" not in spotify_pls:
        print("WARNING: No followed artists found in the backup file.")
        return

    # The structure has been simplified, followed_artists is now a simple list of artist names
    if isinstance(spotify_pls["followed_artists"], list):
        for artist_name in spotify_pls["followed_artists"]:
            yield artist_name
    else:
        print("WARNING: The followed_artists data structure in the backup file is not as expected.")
        return


def subscribe_to_artists(
    ytmusic_artist_channel_ids: Optional[List[str]] = None,
    spotify_playlist_file: str = "playlists.json",
    spotify_encoding: str = "utf-8",
    dry_run: bool = False
) -> None:
    """Subscribe to YouTube Music artist channels based on Spotify followed artists.

    This function will try to find the YouTube Music equivalent of each Spotify artist
    and subscribe to their channel.

    Args:
        `ytmusic_artist_channel_ids` (Optional[List[str]], optional): List of YouTube Music artist channel IDs to subscribe to.
            If None, will try to match Spotify followed artists to YouTube Music. Defaults to None.
        `spotify_playlist_file` (str, optional): The path to the playlists backup file. Defaults to "playlists.json".
        `spotify_encoding` (str, optional): Characters encoding. Defaults to "utf-8".
        `dry_run` (bool, optional): If True, only print actions without actually subscribing. Defaults to False.
    """
    yt = get_ytmusic()
    ...
    if ytmusic_artist_channel_ids:
        # Subscribe to specific artist channel IDs
        for channel_id in ytmusic_artist_channel_ids:
            if not dry_run:
                try:
                    yt.subscribe_artists([channel_id])
                    print(f"Subscribed to artist with channel ID: {channel_id}")
                    artists_subscribed += 1
                except Exception as e:
                    print(f"ERROR: Failed to subscribe to artist channel {channel_id}: {e}")
            else:
                print(f"Would subscribe to artist with channel ID: {channel_id}")
    else:
        # Try to match Spotify artists with YouTube Music artists
        for artist_name in iter_spotify_followed_artists(spotify_playlist_file, spotify_encoding):
            print(f"Looking for YouTube Music equivalent for Spotify artist: {artist_name}")

            # Search for the artist on YouTube Music
            search_results = yt.search(artist_name, filter="artists")

            if not search_results:
                print(f"  Artist '{artist_name}' not found on YouTube Music")
                artists_not_found += 1
                continue

            # Get the first result as the most likely match
            ytm_artist = search_results[0]

            if "browseId" in ytm_artist:
                browse_id = ytm_artist["browseId"]
                artist_name = ytm_artist.get("artist", "Unknown")
                
                try:
                    # Get the artist details to obtain the channelId
                    artist_details = yt.get_artist(browse_id)
                    
                    if "channelId" in artist_details:
                        channel_id = artist_details["channelId"]
                        
                        if not dry_run:
                            try:
                                yt.subscribe_artists([channel_id])
                                print(f"  Subscribed to {artist_name} (channel ID: {channel_id})")
                                artists_subscribed += 1
                            except Exception as e:
                                print(f"  ERROR: Failed to subscribe to {artist_name}: {e}")
                        else:
                            print(f"  Would subscribe to {artist_name} (channel ID: {channel_id})")
                    else:
                        print(f"  Could not find a valid channel ID in artist details for '{artist_name}'")
                        artists_not_found += 1
                except Exception as e:
                    print(f"  ERROR: Failed to get artist details for {artist_name}: {e}")
                    artists_not_found += 1
            else:
                print(f"  Could not find a valid browse ID for '{artist_name}'")
                artists_not_found += 1

    print()
    print(f"Subscribed to {artists_subscribed} artists, could not find {artists_not_found} artists")
